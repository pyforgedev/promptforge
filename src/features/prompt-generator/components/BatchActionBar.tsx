...
import { Download, FileJson, Heart } from 'lucide-react'
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 rounded-lg border border-border bg-card/70 px-4 py-3 shadow-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        {onSaveAll && (
          <Button variant="outline" size="sm" onClick={onSaveAll} className="gap-1.5">
            <Heart className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">{t('batchActionBar.saveAll')}</span>
            <span className="sm:hidden">{t('batchActionBar.save')}</span>
          </Button>
        )}
      </div>
    </div>
